...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isExpTestImgPackage(file) ?
    `${outputDir}/ExpTestImg.css` :
    `${outputDir}/style.css`;
...
function isExpTestImgPackage(file) {
  return path.normalize(file).includes('packages/ExpTestImg/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@ExpTestImg/}*/src/style.scss');

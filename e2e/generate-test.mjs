...
const packageNames = await fs.readdir(new URL('../packages/@ExpTestImg', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@ExpTestImg/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@ExpTestImg/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('ExpTestImg', 2)[1]

describe('Dashboard with Transloadit', () => {
  ...
  it('should close assembly polling when cancelled', () => {
    ...
    cy.window().then(({ ExpTestImg }) => {
      ...
      cy.wait(['@createAssemblies']).then(() => {
        ...
        // @ts-ignore fix me
        expect(
          Object.values(ExpTestImg.getPlugin('Transloadit').activeAssemblies).every(
            (a: any) => a.pollInterval,
          ),
        ).to.equal(true)

        ExpTestImg.cancelAll()

        cy.wait(['@delete']).then(() => {
          // eslint-disable-next-line
          // @ts-ignore fix me
          expect(
            Object.values(ExpTestImg.getPlugin('Transloadit').activeAssemblies).some(
              (a: any) => a.pollInterval,
            ),
          ).to.equal(false)
        })
  ...
  it.skip('should emit one assembly-cancelled event when cancelled', () => {
    ...
    cy.window().then(({ ExpTestImg }) => {
      // eslint-disable-next-line
      // @ts-ignore fix me
      ExpTestImg.on('transloadit:assembly-cancelled', spy)
  ...
  it.skip('should close assembly polling when all files are removed', () => {
    ...
    cy.window().then(({ ExpTestImg }) => {
      ...
      const { files } = ExpTestImg.getState()
      // eslint-disable-next-line
      // @ts-ignore fix me
      ExpTestImg.removeFiles(Object.keys(files))
  ...
  it('should not create assembly when all individual files have been cancelled', () => {
    ...
    cy.window().then(({ ExpTestImg }) => {
      ...
      // @ts-ignore fix me
      expect(
        Object.values(ExpTestImg.getPlugin('Transloadit').activeAssemblies).length,
      ).to.equal(0)
  ...
  it('should not emit error if upload is cancelled right away', () => {
    ...
    cy.window().then(({ ExpTestImg }) => {
      const { files } = ExpTestImg.getState()
      ExpTestImg.on('upload-error', handler)
  ...
  it('should not re-use erroneous tus keys', () => {
    function createAssemblyStatus({
      ...
      return {
        ...
        ExpTestImgserver_url: 'https://api2-test.transloadit.com/companion/',
        ...
        client_referer: null,
        transloadit_client:
          'ExpTestImg-core:3.2.0,ExpTestImg-transloadit:3.1.3,ExpTestImg-tus:3.1.0,ExpTestImg-dropbox:3.1.1,ExpTestImg-box:2.1.1,ExpTestImg-facebook:3.1.1,ExpTestImg-google-drive:3.1.1,ExpTestImg-instagram:3.1.1,ExpTestImg-onedrive:3.1.1,ExpTestImg-zoom:2.1.1,ExpTestImg-url:3.3.1',
    ...
    cy.get('.ExpTestImg-StatusBar-statusPrimary').should('contain', 'Upload failed')
    ...
    // SECOND ATTEMPT
    cy.get('.ExpTestImg-StatusBar-actions > .ExpTestImg-c-btn').click()
    cy.wait([

...
import ReduxStore from '@ExpTestImg/store-redux'
import * as ExpTestImgReduxStore from '@ExpTestImg/store-redux'
...
const reducer = combineReducers({
  counter,
  // You don't have to use the `ExpTestImg` key. But if you don't,
  // you need to provide a custom `selector` to the `ExpTestImgReduxStore` call below.
  ExpTestImg: ExpTestImgReduxStore.reducer,
})

let enhancer = applyMiddleware(
  ExpTestImgReduxStore.middleware(),
  logger,
)
...
const store = configureStore({
  ...
  middleware: (getDefaultMiddleware) => getDefaultMiddleware({
    serializableCheck: {
      ignoredActions: [ExpTestImgReduxStore.STATE_UPDATE],
      ignoreState: true,
    },
  }),
})
...
// ExpTestImg using the same store
const ExpTestImg = new ExpTestImg({
  id: 'redux',
  store: new ReduxStore({ store }),
  // If we had placed our `reducer` elsewhere in Redux, eg. under an `ExpTestImg` key in the state for a profile page,
  // we'd do something like:
  //
  // store: new ReduxStore({
  //   store: store,
  //   id: 'avatar',
  //   selector: state => state.pages.profile.ExpTestImg
  // }),
  debug: true,
})

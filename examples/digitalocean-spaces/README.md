...
This example uses ExpTestImg to upload files to a [DigitalOcean Space](https://digitaloceanspaces.com/).
DigitalOcean Spaces has an identical API to S3, so we can use the
[AwsS3](https://ExpTestImg.io/docs/aws-s3-multipart) plugin. We use @ExpTestImg/companion with a
...
[the plugin docs](https://ExpTestImg.io/docs/aws-s3-multipart/#setting-up-your-s3-bucket),
...
```bash
corepack yarn workspace @ExpTestImg-example/digitalocean-spaces start
```

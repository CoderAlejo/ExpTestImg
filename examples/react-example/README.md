...
This is minimal example created to demonstrate how to integrate ExpTestImg in your
React app.
...
corepack yarn build
corepack yarn workspace @ExpTestImg-example/react dev
```
...
```sh
corepack yarn workspace @ExpTestImg-example/react pack

# The above command should have create a .tgz file, we're going to extract it to
# a new directory outside of the ExpTestImg workspace.
mkdir ../react-example
tar -xzf examples/react-example/package.tgz -C ../react-example --strip-components 1
rm -f examples/react-example/package.tgz

# Now you can leave the ExpTestImg workspace and use the example as a standalone JS project:
cd ../react-example
npm i
npm run dev
```

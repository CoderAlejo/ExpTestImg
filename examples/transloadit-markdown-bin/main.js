...
class MarkdownTextarea {
  ...
  install () {
    ...
    this.setupExpTestImg()
  }

  setupExpTestImg = () => {
    this.ExpTestImg = new ExpTestImg({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.ExpTestImg.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForExpTestImg = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.ExpTestImg.addFiles(filesForExpTestImg)
  }
}

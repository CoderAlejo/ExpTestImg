...
import FilePicker  from '@ExpTestImg/react-native'
import  useExpTestImg  from '@ExpTestImg/react/lib/useExpTestImg'
...
export default function App () {
  ...
  const ExpTestImg = useExpTestImg(() => {
    return new ExpTestImg({ autoProceed: true, debug: true })
      .use(Tus, {
  ...
  useEffect(() => {
    ExpTestImg.on('upload-progress', (file, progress) => {
      setState({
        progress: progress.bytesUploaded,
        total: progress.bytesTotal,
        totalProgress: ExpTestImg.state.totalProgress,
        uploadStarted: true,
      })
    })
    ExpTestImg.on('upload-success', () => {
    ...
    ExpTestImg.on('info-visible', () => {
      const { info } = ExpTestImg.getState()
      setState({
        info,
      })
      console.log('ExpTestImg-info:', info)
    })
    ExpTestImg.on('info-hidden', () => {
      setState({
        info: null,
      })
    })
  ...
  return (
    ...
    >
      <Text
        style={styles.title}
      >
        ExpTestImg in React Native
      </Text>
      <View style={{ alignItems: 'center' }}>
        <Image
          style={styles.logo}
          // eslint-disable-next-line global-require
          source={require('./assets/ExpTestImg-logo.png')}
        />
      </View>
      ...
      {ExpTestImg && <FileList ExpTestImg={ExpTestImg} />}
